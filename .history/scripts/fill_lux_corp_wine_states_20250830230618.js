...
// read taxonomy to extract state keys
const taxRaw = fs.readFileSync(TAX_PATH,'utf8');
const stateKeyRe = /key:\s*['"]([a-z\-]+)['"]\s*,\s*label:\s*['"][^'"]+['"]\s*,\s*group:\s*['"]states['"]/g;
const stateKeys = [];
let m;
while((m = stateKeyRe.exec(taxRaw)) !== null){ stateKeys.push(m[1]); }

const wantKeys = [
  'type:luxury',
  'event:corporate',
  'event:wine',
  ...stateKeys.map(k => k) // state keys are already slugified (e.g., alabama)
];
