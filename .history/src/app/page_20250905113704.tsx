...
// Clean minimal homepage to ensure the app compiles. The DB-driven homepage will be re-enabled after verification.
export default function Home() {
  return (
    <main className="p-8">
      <h1 className="text-2xl font-bold">Bus2Ride</h1>
      <p className="mt-2 text-gray-600">Homepage placeholder — stable build.</p>
    </main>
  );
}
import React from "react";
...
// Minimal server-rendered homepage: fetches page definition from Supabase and renders blocks
...
// Minimal server-rendered homepage: fetch page definition from Supabase and render blocks
export default async function Home() {
  const data = await fetchHomePage();
  if (!data) return <div className="p-8">No page found.</div>;
  return <main>{data.blocks.map((b) => renderBlock(b))}</main>;
