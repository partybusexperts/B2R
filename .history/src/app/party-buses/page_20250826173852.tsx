...
export default function PartyBusesPage() {
  ...
  interface BusWithOpt extends Bus { opt?: ReturnType<typeof findByFileName>; }

  const catalogPartyBuses = useMemo(() => resolveVehicles(findByFileName).filter(v => v.category === 'party-buses'), []);

  const busesWithImages: BusWithOpt[] = useMemo(() => {
    return catalogPartyBuses.map(v => ({
      name: v.name,
      capacity: v.capacityMax,
      type: 'Party Bus',
      highlights: v.highlights,
      badge: v.badge,
      image: v.primary?.file,
      opt: v.primary?.entry || partyOptimized.find(p => p.alt.toLowerCase().includes('party bus'))
    }));
  }, [catalogPartyBuses, partyOptimized]);
