...
/* Use the provided local image for both cards */
const LIMO_IMG = "/images/Bus-1.png";
const SHUTTLE_IMG = "/images/Bus-1.png";
...
export default function PartyBusesPage() {
  ...
  <ul className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
    {PARTY_BUS_FEATURES.map((f, idx) => (
      ...
      {/* ---------- POLLS (fixed to align with the rest) ---------- */}
      <section className="max-w-6xl mx-auto bg-gradient-to-br from-[#0e1a39] to-[#080e1f] rounded-3xl shadow-xl my-12 py-12 px-6 border border-blue-900/40">
        <h2 className="text-4xl md:text-5xl font-extrabold text-center mb-2 text-white font-serif tracking-tight">
          Party Bus Polls
        </h2>
      ...
      <section className="max-w-6xl mx-auto bg-gradient-to-br from-[#0e1a39] to-[#080e1f] rounded-3xl shadow-xl my-12 py-12 px-6 border border-blue-900/40">
        ...
        {/* ---------- HOW IT WORKS (renamed, now with modals) ---------- */}
        <section className="max-w-7xl mx-auto px-4 md:px-6 my-12">
          ...
          {[
            { step: "★1", label: "Contact Us", icon: "📞", href: "/contact", desc: "Reach out by phone, email, or our contact form. We’ll answer questions and help you get started." },
            { step: "★2", label: "Get a Quote", icon: "💬", href: "/quote#instant", desc: "Tell us your trip details—date, group size, pickup/dropoff, and hours. Get a fast, transparent quote." },
            { step: "★3", label: "Reserve Your Ride", icon: "📝", href: "/reserve", desc: "Ready to book? Reserve your vehicle with a deposit. We’ll confirm all details and lock in your date." },
            { step: "★4", label: "Finalize & Ride", icon: "🎉", href: "/itinerary", desc: "Finalize your itinerary, get your driver’s info, and enjoy a smooth, on-time ride with Bus2Ride!" },
          ].map((s, idx) => (
            <div key={s.step} className="relative flex-1">
              {/* Checkbox controls modal */}
              <input
            id={`howitworks-${idx}`}
            type="checkbox"
            className="peer sr-only"
            aria-hidden="true"
              ...
              {/* Card (click to open) */}
              <label
            htmlFor={`howitworks-${idx}`}
            className="group bg-[#0e1a39] border border-blue-900/60 rounded-2xl px-5 py-6 text-center hover:border-blue-400/60 hover:shadow-[0_0_0_2px_rgba(96,165,250,.25)] transition cursor-pointer block"
            ...
            <div className="relative z-10 w-full max-w-md min-h-[300px] bg-gradient-to-br from-[#0c193a] to-[#070e20] border border-blue-900/50 rounded-2xl shadow-2xl">
              ...
              {s.step}. {s.label}
                ...
                <div className="text-blue-200 text-lg mb-4">{s.desc}</div>
                <a
              href={s.href}
              className="inline-block bg-blue-700 hover:bg-blue-800 text-white font-bold px-8 py-3 rounded-xl shadow border border-blue-800 transition"
                >
              Go to {s.label}
                </a>
              </div>
        ...
        <div className="bg-[#0b142b] border border-blue-900/40 rounded-3xl shadow-xl px-6 md:px-8 py-10">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filteredEvents.map((ev) => (
              <div
                key={ev.title}
                className="relative rounded-2xl overflow-hidden border border-blue-900/40 shadow-lg bg-[#0e1a39] flex flex-col"
              >
                {/* Image */}
                ...
                <div className="absolute inset-x-0 top-0 p-5 bg-gradient-to-b from-black/55 via-black/25 to-transparent pointer-events-none">
                ...
                <div className="p-5">
                  <div className="flex flex-col gap-2">
                    <a
                      href={`tel:${PHONE_TEL}`}
                      className="inline-flex items-center justify-center rounded-xl px-4 py-2.5 font-bold bg-blue-700 text-white hover:bg-blue-800 border border-blue-800 transition"
                    ...
                    <a
                      href={`mailto:${EMAIL}`}
                      className="inline-flex items-center justify-center rounded-xl px-4 py-2.5 font-bold bg-blue-900 text-white hover:bg-blue-950 border border-blue-900 transition"
                    >
                      Email Now
                    </a>
                    <a
