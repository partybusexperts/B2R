  ...
  const eventsWithImages = useMemo(
    () =>
      POPULAR_EVENT_TITLES.map((title, i) => ({
        ...
        optimizedEntry: partyOptimized[i % (partyOptimized.length || 1)]
      })),
    [partyOptimized]
  );
    ...
    <main className="text-slate-100 bg-[#0f1f46]">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 my-12">
        <div className="bg-[#122a56] border border-blue-800/30 rounded-3xl shadow-xl px-5 md:px-8 py-8">
          ...
          <p className="text-blue-200 text-center max-w-3xl mx-auto mt-3 mb-6">Helpful, client-side utilities to plan budgets, BYOB, stops, and group sizes.</p>

          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {TOOL_LIST.map((t, i) => (
              <button key={t.name} onClick={() => setActiveToolIdx(i)} className="text-left bg-[#12244e] rounded-2xl p-4 border border-blue-800/30 hover:scale-105 transition-transform">
