  ...
  return (
    <main className="text-white bg-[#05070f]">
      ...
      <section className="max-w-6xl mx-auto bg-gradient-to-br from-[#0e1a39] to-[#080e1f] rounded-3xl shadow-xl my-12 py-12 px-6 border border-blue-900/40">
        ...
        <li
          key={f.label}
          className="group bg-[#0b142b] rounded-2xl shadow border border-blue-900/40 px-5 py-4 text-blue-100 flex items-start gap-3 cursor-pointer hover:scale-105 transition-transform"
          tabIndex={0}
          role="button"
          aria-label={`Learn more: ${f.label}`}
          onClick={() => setActiveToolIdx(1000 + idx)}
          onKeyDown={e => {
            if (e.key === "Enter" || e.key === " ") setActiveToolIdx(1000 + idx);
          }}
        >
        ...
        {/* Feature Modal */}
        {typeof window !== "undefined" && activeToolIdx !== null && activeToolIdx >= 1000 && activeToolIdx < 1000 + PARTY_BUS_FEATURES.length && (
          <div
        ...
        onClick={closeModal}
          >
        <div
          className="w-full max-w-md min-h-[300px] bg-gradient-to-br from-[#0c193a] to-[#070e20] border border-blue-900/50 rounded-2xl shadow-2xl relative"
        ...
        >
          ...
          {PARTY_BUS_FEATURES[activeToolIdx - 1000].icon}
            </div>
            <h3 className="text-2xl font-extrabold text-white mb-2 font-serif tracking-tight">
          {PARTY_BUS_FEATURES[activeToolIdx - 1000].label}
            </h3>
            <div className="text-blue-200 text-lg">{PARTY_BUS_FEATURES[activeToolIdx - 1000].description}</div>
          </div>
        </div>
          </div>
        )}
