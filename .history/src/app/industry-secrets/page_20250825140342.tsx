...
export default function IndustrySecretsPage(): JSX.Element {
  ...
  const filtered = React.useMemo(() => {
    ...
    return INDUSTRY_SECRETS.filter((s) => {
      const hay = `${s.title} ${s.teaser} ${s.details} ${s.tag ?? ""}`.toLowerCase();
      return hay.includes(needle);
    });
  }, [q]);

  // Helpful banners we’ll sprinkle between rows
  const banners = [
    {
      title: "Get an ALL-IN Quote Checklist",
      body: "When you price-compare, make sure your quote includes base hourly, taxes, fuel, admin, tolls, cleaning, and gratuity. Apples-to-apples saves you $$$.",
      ctaText: "See Pricing",
      href: "/pricing",
    },
    {
      title: "Prom & Peak Saturdays = Plan Early",
      body: "If you can do Sun–Thu or daytime, you’ll unlock better vehicles and lower minimums. Flexibility is your secret weapon.",
      ctaText: "Ask About Off-Peak",
      href: "/contact",
    },
    {
      title: "Not Sure on Size?",
      body: "‘30 passenger’ is marketing max. For coolers and dance space, size up one class for comfort.",
      ctaText: "Explore Fleet",
      href: "/party-buses",
    },
  ];

  // Build the grid with a banner after every 6 cards
  const gridItems: React.ReactNode[] = [];
  filtered.forEach((secret, i) => {
    gridItems.push(<SecretCard key={secret.id} secret={secret} onOpen={openSecret} />);
    const isBreakpoint = (i + 1) % 6 === 0 && i !== filtered.length - 1;
    if (isBreakpoint) {
      const banner = banners[((i + 1) / 6 - 1) % banners.length];
      gridItems.push(
        <InfoBanner
          key={`banner-${i}`}
          title={banner.title}
    ...
    <main className="bg-gradient-to-br from-blue-50 via-white to-blue-100 border border-blue-200 rounded-3xl shadow-xl max-w-6xl mx-auto my-10 p-6 md:p-10">
      <header className="text-center mb-8 md:mb-10">
        ...
        <p className="mt-3 text-blue-900/80 max-w-3xl mx-auto">
          Insider tips operators don’t always advertise—use these to book smarter,
          avoid surprise fees, and get the most out of your ride.
        </p>
        ...
        >
          <div className="relative w-full max-w-2xl">
            <input
              ...
              className="w-full rounded-2xl border border-blue-300 bg-white/90 px-5 py-3 pr-24 shadow-sm outline-none placeholder:text-blue-900/40 focus:border-blue-500 focus:ring-2 focus:ring-blue-300"
            />
            {q && (
              <button
                type="button"
      ...
      <footer className="mt-12 text-center">
        <div className="text-sm text-blue-900/70">
          Got questions?{" "}
          <Link href="/contact" className="font-semibold text-blue-800 underline-offset-2 hover:underline">
            Contact us
          </Link>{" "}
          or{" "}
          <Link href="/pricing" className="font-semibold text-blue-800 underline-offset-2 hover:underline">
            check pricing
          </Link>
          .
        </div>
        <div className="mt-2">
          <Link
            href="/"
            className="inline-block mt-2 rounded-xl bg-white border border-blue-300 px-4 py-2 text-blue-800 font-semibold hover:bg-blue-50"
          >
            ← Back to Home
          </Link>
        </div>
