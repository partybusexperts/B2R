  ...
  {/* Pricing table removed per request */}
      ...
      <Section className="max-w-7xl mx-auto bg-gradient-to-br from-blue-900/80 to-black rounded-3xl shadow-xl border border-blue-500/30 py-14 px-6 mb-16">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {anchoragePolls.length === 0 && (
            <div className="col-span-full text-blue-200 text-center py-8">No Anchorage/Alaska-specific polls found in the registry.</div>
          )}
          {anchoragePolls.map((p) => (
            <div key={p.id} className="bg-[#132a55] rounded-2xl p-5 border border-blue-700/40 shadow flex flex-col">
              <div className="font-bold text-blue-50 mb-2 text-sm leading-snug">{p.question}</div>
              <ul className="text-[11px] text-blue-200 space-y-1">
                {Array.isArray(p.options) && p.options.map((o) => (
                  <li key={o} className="bg-blue-800/30 rounded px-2 py-1 inline-block">{o}</li>
                ))}
              </ul>
            </div>
          ))}
