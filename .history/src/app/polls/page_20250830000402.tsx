...
/**
 * Server Component: reads the canonical polls registry JSON at build time
 * and renders a lightweight client component for interactive polls.
 */
            ...
            return (
              <div key={tag} id={`board-${slug(tag)}`} className="bg-gradient-to-br from-[#122a5c] to-[#0f2148] rounded-3xl shadow-xl p-6 border border-blue-800/30">
                ...
                {rest.length > 0 && (
                   ...
                   */
                  export default async function PollsPage() {
                    const filePath = path.join(process.cwd(), 'src', 'data', 'pollsRegistry.json');
                    const raw = await fs.readFile(filePath, 'utf8');
                    const polls = JSON.parse(raw);
                    return <ClientPolls polls={polls} />;
                  }
