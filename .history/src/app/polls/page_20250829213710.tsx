...
const pretty = (s: string) => s.split(/[-_]/g).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
...
function useToast() { const [msg, setMsg] = useState<string | null>(null); useEffect(() => { if (!msg) return; const t = setTimeout(() => setMsg(null), 1800); return () => clearTimeout(t); }, [msg]); return { msg, push: setMsg }; }

function AutoComplete({ polls, value, onChange, onSelect, }: { polls: Poll[]; value: string; onChange: (v: string) => void; onSelect: (p: Poll) => void; }) {
  ...
  const suggestions = useMemo(() => {
    ...
    if (!q) return [];
    return polls.filter(p => p.question.toLowerCase().includes(q) || (p.tags || []).join(" ").toLowerCase().includes(q)).slice(0, 8);
  }, [value, polls]);

  useEffect(() => { setIdx(0); setOpen(suggestions.length > 0); }, [suggestions.length]);
      ...
      <section className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {boards.map((board) => {
            ...
            const featured = board.featured.filter(p => !selectedTag || (p.tags || []).map(x=>x.toLowerCase()).includes(selectedTag.toLowerCase())).filter(p => { const q = query.trim().toLowerCase(); if (!q) return true; return p.question.toLowerCase().includes(q) || (p.tags||[]).some(t => pretty(t).toLowerCase().includes(q)); });
            const rest = board.rest.filter(p => !selectedTag || (p.tags || []).map(x=>x.toLowerCase()).includes(selectedTag.toLowerCase())).filter(p => { const q = query.trim().toLowerCase(); if (!q) return true; return p.question.toLowerCase().includes(q) || (p.tags||[]).some(t => pretty(t).toLowerCase().includes(q)); });
