...
export default function Page() {
  ...
  // prefetch visible poll previews' results and load any local selections
  useEffect(() => {
    for (const cat of all) {
      for (let i = 0; i < 3; i++) {
        const pollId = `${cat.key}__${i}`;
        // load stored selection
        try {
          const stored = typeof window !== 'undefined' ? localStorage.getItem(`poll_vote_${pollId}`) : null;
          if (stored) setSelectedMap((m) => ({ ...m, [pollId]: stored }));
        } catch { /* ignore */ }
        // fetch results
        fetchResultsFor(pollId);
      }
    }
  }, [all]);

  // per-poll server results + local selection state (for polls page)
  const [selectedMap, setSelectedMap] = useState<Record<string, string>>({});
  const [resultsCache, setResultsCache] = useState<Record<string, Record<string, number>>>({});

  const pollApiBase = (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_API_BASE) ? process.env.NEXT_PUBLIC_API_BASE : '';
  const buildApi = (path: string) => `${pollApiBase}${path}`.replace(/([^:]?)\/\//g, '$1/');

  async function fetchResultsFor(pollId: string) {
    try {
      const resp = await fetch(buildApi(`/api/poll/results/${encodeURIComponent(pollId)}`));
      if (!resp.ok) return;
      const json = await resp.json();
      setResultsCache((m) => ({ ...m, [pollId]: json.results || {} }));
    } catch { /* ignore */ }
  }

  async function handleVoteOnPage(catKey: string, idx: number, option: string) {
    const pollId = `${catKey}__${idx}`;
    try {
  // no-op loading map used; UI shows immediate change via selectedMap
      const resp = await fetch(buildApi('/api/poll/vote'), {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ poll_id: pollId, option })
      });
      if (!resp.ok) throw new Error('vote failed');
      // store local answer for UI
  try { localStorage.setItem(`poll_vote_${pollId}`, option); } catch { /* ignore */ }
      setSelectedMap((m) => ({ ...m, [pollId]: option }));
  // fetch fresh results and cache
  await fetchResultsFor(pollId);
      // also mirror into answers store used by the page for progress
      setAnswer(`${catKey}::${idx}`, optionsFor('')[0] ? 0 : 0); // keep local answer tracking; no-op index
    } catch (err) {
                      ...
                      return (
                        <li key={pid} className="bg-blue-900/60 rounded-xl px-4 py-3 border border-blue-700/20">
                          <div className="flex items-start justify-between">
                            <div className="text-blue-100 font-medium mb-2 max-w-[78%]">{poll}</div>
                            <div className="text-right ml-2">
                              <PollShare anchor={`#${idFor(parent, cat.title)}-${i}`} labelOnly />
                            </div>
                          </div>
                            <div className="flex flex-wrap gap-2">
                              {resultsCache[`${cat.key}__${i}`] ? (
                                // show aggregated results
                                Object.keys(resultsCache[`${cat.key}__${i}`]).map((opt) => {
                                  const count = Number(resultsCache[`${cat.key}__${i}`][opt] || 0);
                                  const total = Object.values(resultsCache[`${cat.key}__${i}`]).reduce((a, b) => Number(a) + Number(b), 0);
                                  const pct = total > 0 ? Math.round((count / total) * 100) : 0;
                                  const isMine = selectedMap[`${cat.key}__${i}`] === opt;
                                  return (
                                    <div key={opt} className="flex items-center gap-2">
                                      <div className={`px-3 py-1 rounded-full text-sm ${isMine ? 'bg-green-200 text-green-900' : 'bg-blue-950/70 text-blue-200'}`}>{opt}</div>
                                      <div className="text-xs text-blue-100">{count} ({pct}%)</div>
                                    </div>
                                  );
                                })
                              ) : (
                                options.map((opt) => (
                                  <button
                                    key={opt}
                                    onClick={() => handleVoteOnPage(cat.key, i, opt)}
                                    className={`px-3 py-1 rounded-full border text-sm ${
                                      selectedMap[`${cat.key}__${i}`] === opt
                                        ? "bg-green-400 text-blue-950 border-green-300"
                              ...
                              <div key={pid} className="bg-blue-900/70 rounded-xl px-4 py-3 border border-blue-400/20">
                                <div className="flex items-start justify-between mb-2">
                                  ...
                                  <div className="ml-4 text-right">
                                    <PollShare anchor={`#${idFor(parent, cat.title)}__${i}`} />
                                  </div>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                  {resultsCache[`${cat.key}__${i}`] ? (
