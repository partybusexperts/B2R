...
function takeFirst<T>(arr: T[], count: number) { return arr.slice(0, count); }

function useRandomSubset<T>(arr: T[], count: number) {
  const [subset, setSubset] = useState<T[]>(() => takeFirst(arr, count));
  useEffect(() => {
    const shuffled = [...arr];
    ...
    setSubset(shuffled.slice(0, count));
  }, [arr, count]);
  return subset;
}
...
export default function Home() {
  ...
  const partyShow = useRandomSubset(partyOptimizedAll, 3);
  const limoShow = useRandomSubset(limoOptimizedAll, 3);
  const coachShow = useRandomSubset(coachOptimizedAll, 3);
  return (
  ...
  <div className="mt-6 flex flex-wrap md:flex-nowrap gap-2">
    ...
    <div className="relative">
      ...
      <div className="relative group rounded-[24px] overflow-hidden border border-blue-800/40 bg-[#0f2148] shadow-[0_14px_40px_rgba(2,6,23,.55)]">
        <div className="relative">
          <SmartImage
            src="/images/18 Passenger Party Bus White Exterior.png"{/* higher-res (1067px wide) variant to avoid upscaling blur */}
            ...
            sizes="(max-width:768px) 100vw, 540px" /* explicit sizes so Next picks correct DPR variants */
          />
          {/* gloss sweep */}
        ...
        >
          ...
          <div className="px-6 mt-5">
            ...
            <ul className="text-blue-100/95 text-[0.95rem] space-y-1 min-h-[72px]">
              {["Premium sound", "LED lighting", "BYOB-friendly"].map((h) => (
      ...
      key={entry.original || idx}
        ...
        >
          ...
          <div className="px-6 mt-5">
            ...
            <ul className="text-blue-100/95 text-[0.95rem] space-y-1 min-h-[72px]">
              {["Leather seating", "Mood lighting", "Premium sound"].map((h) => (
                <li key={h} className="flex items-start gap-2">
                  <span className="mt-[2px]">•</span>
                  <span>{h}</span>
                </li>
...
<Section className="max-w-6xl mx-auto">
      ...
      key={entry.original || idx}
        ...
        >
          ...
          <div className="px-6 mt-5">
            ...
            <ul className="text-blue-100/95 text-[0.95rem] space-y-1 min-h-[72px]">
              {["Reclining seats", "Wi-Fi & charging", "Luggage bays"].map((h) => (
                <li key={h} className="flex items-start gap-2">
                  <span className="mt-[2px]">•</span>
                  <span>{h}</span>
                </li>
  ...
  {/* Trust & Reputation */}
<Section className="relative bg-gradient-to-br from-blue-900/80 to-black overflow-hidden">
  {/* soft glow orbs */}
  ...
  <div className="max-w-5xl mx-auto px-4 text-center relative">
    <h2 className="text-4xl md:text-5xl font-extrabold text-center mb-3 mt-10 bg-gradient-to-r from-white via-blue-200 to-blue-500 bg-clip-text text-transparent drop-shadow-lg tracking-tight">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
      {[
        { t: "4.9★", s: "Average Rating" },
        { t: "10,000+", s: "Happy Riders" },
        { t: "On-Time", s: "Promise" },
        { t: "100%", s: "Upfront Pricing" },
      ].map((x, i) => (
        <div
          key={i}
        ...
        >
          ...
          <div className="text-blue-700 text-xs font-semibold uppercase tracking-wide">{x.s}</div>
        </div>
      ))}
    </div>
