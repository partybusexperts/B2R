...
export default function PollResultsPage() {
  ...
  const entries = useMemo(() => {
    // map tag -> rows
    const itemsByKey: Record<string, typeof grouped[0] | any> = {};
    for (const g of grouped) itemsByKey[g.category] = (itemsByKey[g.category] || []).concat(g.rows.map((r: any) => r.poll));

    const out: { slug: string; raw: string; pretty: string; items: any[]; count: number }[] = [];
