...
function TrustStatModalCard({ stat, label, title, content }: { stat: string; label: string; title: string; content: React.ReactNode }) {
  return (
    <div className="bg-white/95 text-blue-900 rounded-2xl border border-blue-200 shadow p-4 hover:-translate-y-0.5 transition will-change-transform flex flex-col">
      <button
        type="button"
        className="text-left flex-1"
        onClick={(e) => {
          e.preventDefault();
          // Delegate to PartyBusFeatureModalButton pattern by reusing it inline
        }}
      >
        <div className="text-2xl font-extrabold tracking-tight mb-0.5">{stat}</div>
        <div className="text-blue-700 text-xs font-semibold uppercase tracking-wide">{label}</div>
      </button>
      {/* Hidden actual modal trigger using PartyBusFeatureModalButton for consistent modal UI */}
      <div className="hidden">
        <PartyBusFeatureModalButton label={`${stat} ${label}`} title={title} content={content} />
      </div>
    </div>
  );
}
