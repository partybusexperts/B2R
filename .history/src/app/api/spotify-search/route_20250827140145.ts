...
export async function POST(req: NextRequest) {
  try {
    ...
    const items = (j.playlists?.items || []).map((pl: any) => ({
      id: pl.id as string,
      name: pl.name as string,
      description: (pl.description as string || "").replace(/<[^>]+>/g, ""),
      image: pl.images?.[0]?.url as string | undefined,
      url: pl.external_urls?.spotify as string | undefined,
    }));
