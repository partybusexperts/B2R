...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ ok: false, error: "spotify_auth_failed", message: "Server missing or invalid Spotify credentials" }, { status: 500 });
    }
  const params = new URLSearchParams({ q, type: "playlist", limit: "8", market: 'US' });
  ...
  const items = rawItems.map((pl) => {
    ...
    url: pl.external_urls?.spotify || (id ? `https://open.spotify.com/playlist/${id}` : undefined),
      };
    });
