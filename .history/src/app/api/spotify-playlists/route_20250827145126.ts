...
export async function POST(req: NextRequest) {
  try {
    ...
    const results: Record<string, { id: string; name: string; description: string; image?: string; url?: string; fetched: boolean; status: number; }> = {};
    for (const id of uniqueIds) {
      try {
        ...
        if (!r.ok) {
          ...
          results[id] = { id, name: id, description: "", image: undefined, url: `https://open.spotify.com/playlist/${id}` , fetched: false, status: r.status };
          continue;
        }
        ...
        results[id] = { id, name: id, description: "", image: undefined, url: `https://open.spotify.com/playlist/${id}` , fetched: false, status: 500 };
      }
    }
