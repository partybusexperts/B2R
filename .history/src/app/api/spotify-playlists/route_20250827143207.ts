...
export async function POST(req: NextRequest) {
  try {
    ...
    const results: Record<string, { id: string; name: string; description: string; image?: string; url?: string; }> = {};
    for (const id of uniqueIds) {
      try {
        ...
        if (!r.ok) {
          ...
          results[id] = { id, name: id, description: "Unavailable", image: undefined, url: `https://open.spotify.com/playlist/${id}` };
          continue;
        }
        ...
        results[id] = { id, name: id, description: "Error", image: undefined, url: `https://open.spotify.com/playlist/${id}` };
      }
    }
