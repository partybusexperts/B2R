...
export async function GET(req: NextRequest) {
  ...
  const features = Array.isArray((j as unknown as { features?: unknown }).features) ? (j as any).features : [];
  const suggestions: string[] = features.map((f: unknown) => (f as any).place_name).filter(Boolean);
      return Response.json({ ok: true, suggestions } satisfies SuggestResponse, {
      ...
      const j = await r.json();
      const features2 = Array.isArray((j as unknown as { features?: unknown }).features) ? (j as any).features : [];
      const suggestions: string[] = features2
        .map((f: unknown) => (f as any).properties)
        .filter(Boolean)
        .map((p: any) => {
    ...
    return Response.json(
      { ok: false, error: message || "Geocode suggest failed" } as SuggestResponse,
      { status: 500 }
    );
  }
}
