...
/* ---------------- Mini Planner Helpers ---------------- */
function clamp(n: number, lo: number, hi: number) {
  return Math.max(lo, Math.min(hi, n));
}
function vehicleSuggestion(size: number) {
  if (size <= 14) return { label: "Sprinter Van (up to ~14)", key: "sprinter" };
...
function calcRecommendedHours(stops: number, avgQueueMin: number, travelMin: number, dwellMin: number) {
  ...
  return clamp(hours, 3, 10); // most haunted nights fit 3–10h
}

/* ---------------- Page ---------------- */
export default function HauntedHouseToursPage() {
  // Quick Planner state
  const [groupSize, setGroupSize] = useState(18);
  ...
  const quoteHref = useMemo(() => {
    const params = new URLSearchParams({
      hrs: String(recHours),
      group: String(groupSize),
      event: "haunted-house-tour",
    }).toString();
    ...
    <main className="text-slate-100 bg-[#0f1f46] min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-10">
        <div className="rounded-3xl border border-blue-800/30 bg-[#173264] p-6 shadow-xl text-center">
          ...
          <a
            href={`/polls?tag=${encodeURIComponent("haunted-house")}`}
