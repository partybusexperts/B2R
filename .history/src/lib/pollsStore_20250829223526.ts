...
export async function getAll(): Promise<PollsData> {
  ...
  // Exclude polls that were auto-generated by the filler script (ids like 'autofill_*'
  // or questions beginning with "Auto-fill poll") so the UI shows only real polls.
  const polls = POLL_REGISTRY.filter(p => {
    if (p.active === false) return false;
    if (typeof p.id === 'string' && /^autofill_/i.test(p.id)) return false;
    if (typeof p.question === 'string' && /^auto-?fill poll/i.test(p.question)) return false;
    return true;
  });
  return { polls, votes };
}
