  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Query the content_points table for a matching page_slug.
        // We use the REST endpoint so this works client-side without additional deps.
        const q = `${url.replace(/\/$/, "")}/rest/v1/content_points?select=*&page_slug=eq.${encodeURIComponent(pageSlug)}&limit=1`;
