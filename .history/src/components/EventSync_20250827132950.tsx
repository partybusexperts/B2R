...
export default function EventSync() {
  ...
  // Hydration-safe defaults: render placeholder first (same SSR/client) then populate after mount
  ...
  useEffect(() => {
    const now = Date.now();
    setStart(toDateTimeLocal(new Date(now + 86400000)));
    setEnd(toDateTimeLocal(new Date(now + 90000000)));
  }, []);
