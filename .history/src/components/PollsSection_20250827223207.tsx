...
export function Poll({ poll, initialResults }: { poll: PollType; initialResults?: PollResults }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!aborted) setResults(json.results || {});
      } catch (e) {
  ...
  const handleVote = async (option: string) => {
    ...
    try {
      ...
      try { if (typeof window !== 'undefined') localStorage.setItem(`poll_vote_${poll.id}`, option); } catch {}
    } catch (e) {
      setError('vote');
    } finally {
      setLoading(false);
...
export default function PollsSection() {
  const [allResults, setAllResults] = useState<Record<string, PollResults>>({});
  useEffect(() => {
    let aborted = false;
    (async () => {
      try {
