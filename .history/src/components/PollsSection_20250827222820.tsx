...
export function Poll({ poll, initialResults }: { poll: PollType; initialResults?: Record<string, number> }) {
  ...
  // We always call relative endpoints under /api/poll — next.config rewrites proxy to POLL_TARGET.
  const [activeBase, setActiveBase] = useState<string | null>(null);
  const buildUrl = (base: string, path: string) => `${base}${path}`.replace(/([^:]?)\/\//g, '$1/');
  const baseCandidates = ['/api/poll']; // Simplified to always use relative API endpoints
    ...
    (async () => {
      ...
      for (const candidate of list) {
        try {
          ...
          const url = `/api/poll/results/${encodeURIComponent(poll.id)}`; // Updated to use relative endpoint
  ...
  const resp = await fetch(buildUrl(base, '/api/poll/vote'), {
      ...
      const resp = await fetch(`/api/poll/vote`, { // Updated to use relative endpoint
  ...
  {/* Removed duplicate 'Limo Industry Polls' header to avoid double rendering */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
        {POLL_QUESTIONS.map((poll) => (
          <Poll key={poll.id} poll={poll} initialResults={allResults[poll.id] || {}} />
        ))}
      </div>
