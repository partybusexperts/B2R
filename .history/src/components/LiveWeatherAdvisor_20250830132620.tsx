...
const LiveWeatherAdvisor: React.FC<LiveWeatherAdvisorProps> = ({ fixedPlace, variant = "default" }) => {
  ...
  // NOTE: Read persisted prefs only after mount to avoid SSR/CSR divergence.
  ...
  const next12 = useMemo(() => {
    // Defer until mountNow is set so server + initial client markup (empty list) match.
    if (mountNow == null || !wx?.hourly?.time?.length) return [] as { t: string; temp: number; pop?: number }[];
    const nowRef = mountNow;
    const items: { t: string; temp: number; pop?: number }[] = [];
    for (let i = 0; i < wx.hourly.time.length && items.length < 12; i++) {
  ...
  const pm25Now = useMemo(() => {
    if (mountNow == null || !aq?.hourly?.time?.length || !aq.hourly.pm2_5) return undefined;
    const nowRef = mountNow;
    let idx = 0;
    for (let i = 0; i < aq.hourly.time.length; i++) {
      const ts = new Date(aq.hourly.time[i]).getTime();
      if (ts >= nowRef) { idx = i; break; }
    }
    return aq.hourly.pm2_5?.[idx];
  }, [aq, mountNow]);

  const aqiNow = useMemo(() => {
    if (mountNow == null || !aq?.hourly?.time?.length || !aq.hourly.us_aqi) return undefined;
    const nowRef = mountNow;
    let idx = 0;
    for (let i = 0; i < aq.hourly.time.length; i++) {
      const ts = new Date(aq.hourly.time[i]).getTime();
  ...
  // After mount: load preference values (unit + speed) once.
  useEffect(() => {
    try {
      if (typeof window === "undefined") return;
      const storedUnit = localStorage.getItem("wx:unit");
      if (storedUnit === "C" || storedUnit === "F") setUnit(storedUnit);
      const storedSpeed = localStorage.getItem("wx:speed");
      if (storedSpeed === "mph" || storedSpeed === "km/h") setSpeedUnit(storedSpeed);
    } catch {/* ignore */}
  }, []);
                    ...
                    <div className="text-lg" title={WMO_DESC[wx.daily?.weathercode?.[i] ?? 0] || ""}>
                      {wmoToIcon(wx.daily?.weathercode?.[i] ?? 0)}
                        ...
                        <div className="w-40 text-right">
                          <span className="font-semibold">{wx.daily?.temperature_2m_max?.[i] !== undefined ? showTemp(wx.daily.temperature_2m_max[i]) : '-'}</span>
                          <span className="text-gray-600"> / {wx.daily?.temperature_2m_min?.[i] !== undefined ? showTemp(wx.daily.temperature_2m_min[i]) : '-'}</span>
                        </div>
                  </div>
                ))}
              </div>
      ...
      <div className="relative" ref={suggestBoxRef}>
        <div className="flex items-center border border-gray-400 rounded-xl bg-white shadow overflow-hidden">
          ...
          <input
            ...
            autoComplete="off"
            data-lpignore="true" /* prevent password managers (e.g., LastPass) injecting nodes pre-hydration */
          />
          {query && (
