...
export default function HomePolls({ groups }: { groups?: { tag: string; label?: string }[] }) {
  ...
  const candidates = Array.from(tagCount.entries()).filter(([tag, count]) => count >= 8).map(([tag]) => tag);
  // Fallback: tags with >=4 if not enough
  const fallback = Array.from(tagCount.entries()).filter(([tag, count]) => count >= 4).map(([tag]) => tag);
