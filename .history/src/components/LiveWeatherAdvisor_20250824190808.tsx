...
// Inline emoji icons so no external package is required
const Icon = {
...
interface WeatherApiResponse {
  ...
  current?: {
    ...
    wind_speed_10m?: number; // mph (requested)
  };
  hourly?: {
    time: string[];
    temperature_2m: number[];
...
const LiveWeatherAdvisor: React.FC = () => {
  ...
  // ===== First load: saved place -> geolocation -> Wheaton, IL fallback =====
  useEffect(() => {
    const boot = async () => {
      try {
        setLoading(true);
  ...
  const TempSparkline: React.FC<{ values: number[] }> = ({ values }) => {
    ...
    return (
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-24">
        <polyline fill="none" stroke="currentColor" strokeWidth="2" points={pts.join(" ")} className="text-sky-600" />
      </svg>
    );
  };
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative" ref={suggestBoxRef}>
            <div className="flex items-center border border-gray-300 rounded-xl bg-white shadow-sm overflow-hidden">
              <span className="mx-2 text-gray-500"><Icon.SearchIcon /></span>
              <input
                value={query}
            ...
            {suggestions.length > 0 && (
              <div className="absolute z-20 mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
                {suggestions.map((s, i) => (
                  ...
                  >
                    <span className="text-sky-600"><Icon.MapPin /></span>
                    <span>{s.name}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="md:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                {next12.map((h, i) => (
                  <div key={i} className="rounded-xl border border-gray-200 p-2 text-gray-800">
                    ...
                    {typeof h.pop === "number" && (
                      <div className="text-xs inline-flex items-center gap-1 text-sky-700"><Icon.CloudRain /> {h.pop}%</div>
                    )}
                  </div>
                ))}
              </div>
