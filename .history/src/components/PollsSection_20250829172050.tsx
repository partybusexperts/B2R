...
export default function PollsSection() {
  ...
  const [serverPolls, setServerPolls] = useState<any[] | null>(null);
  useEffect(() => {
    let aborted = false;
    (async () => {
      try {
        ...
        if (!res.ok) return;
        const json = await res.json();
        // expect shape: { polls: [...], votes: {...} }
        if (!aborted) {
          setServerPolls(Array.isArray(json.polls) ? json.polls : []);
          setAllResults(json.votes || {});
        }
      } catch (e) { /* ignore */ }
    })();
    return () => { aborted = true; };
  ...
  // choose a random subset to show on any given page (4 polls)
  const pollsToShow = React.useMemo(() => {
    const base = serverPolls && serverPolls.length ? serverPolls.map(p => ({ id: p.id, question: p.question, options: p.options })) : POLL_QUESTIONS;
