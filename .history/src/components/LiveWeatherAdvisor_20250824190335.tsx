...
interface WeatherApiResponse {
  ...
  current?: {
    ...
    wind_speed_10m?: number; // will request mph
  };
  hourly?: {
    time: string[];
    temperature_2m: number[];
    ...
    uv_index?: number[];
    wind_speed_10m?: number[]; // mph if requested
  };
  daily?: {
    time: string[];
...
const LiveWeatherAdvisor: React.FC = () => {
  ...
  // ===== First load: use saved place -> geolocation -> fallback to Wheaton, IL =====
  useEffect(() => {
    const boot = async () => {
      try {
        setLoading(true);
  ...
  const fetchForecast = async (p: Place): Promise<WeatherApiResponse> => {
    const params = new URLSearchParams({
      ...
      timezone: "auto",
      temperature_unit: "celsius", // store as C internally; display toggle converts
      windspeed_unit: "mph",
    });
  ...
  const pm25Now = useMemo(() => {
    ...
    // pick the entry with the closest time >= now
    const now = Date.now();
    let idx = 0;
  ...
  // ===== Actions =====
  const onSelectSuggestion = async (p: Place) => {
    setQuery("");
    setSuggestions([]);
    await loadAllForPlace(p);
  };
  ...
  const toggleTempUnit = () => {
    ...
    setUnit(next);
    if (typeof window !== "undefined") localStorage.setItem("wx:unit", next);
  };
  const toggleSpeedUnit = () => {
    const next = speedUnit === "mph" ? "km/h" : "mph";
    setSpeedUnit(next);
    if (typeof window !== "undefined") localStorage.setItem("wx:speed", next);
  };

  // ===== Tiny SVG line for next-12h temps =====
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 bg-gradient-to-b from-sky-50 to-sky-100 min-h-screen">
      {/* Header */}
        ...
        <div>
          ...
          {wx?.daily?.sunrise?.[0] && wx?.daily?.sunset?.[0] && (
            <p className="text-sky-800/80 mt-1">
              <span className="inline-flex items-center gap-1 mr-3"><Sun className="w-4 h-4" /> {fmtDate(wx.daily.sunrise[0], { hour: "numeric", minute: "2-digit" })}</span>
              <span className="inline-flex items-center gap-1"><Moon className="w-4 h-4" /> {fmtDate(wx.daily.sunset[0], { hour: "numeric", minute: "2-digit" })}</span>
            </p>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative" ref={suggestBoxRef}>
            <div className="flex items-center border border-gray-300 rounded-xl bg-white shadow-sm overflow-hidden">
              <SearchIcon className="w-5 h-5 mx-2 text-gray-500" />
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search city, state, or country"
                className="px-2 py-2 w-64 md:w-80 outline-none text-gray-800"
              />
              {query && (
                <button
              ...
              <div className="absolute z-20 mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
                {suggestions.map((s, i) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 hover:bg-sky-50 flex items-center gap-2"
                  >
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl bg-white border border-gray-300 shadow hover:bg-gray-50"
          >
      ...
      {/* Status */}
      {loading && (
        <div className="mt-8 text-center text-gray-700 animate-pulse">Loading weather data…</div>
      )}
      {error && (
      ...
      {wx && !loading && !error && (
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-2xl shadow p-4 md:col-span-1">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 text-gray-800">
              <div className="flex items-center gap-2"><Thermometer className="w-4 h-4 text-sky-700" /> <span className="font-medium">{showTemp(currentTemp)}</span></div>
              {typeof currentApparent === "number" && (
                <div className="flex items-center gap-2"><Gauge className="w-4 h-4 text-sky-700" /> Feels {showTemp(currentApparent)}</div>
              )}
              <div className="flex items-center gap-2"><Wind className="w-4 h-4 text-sky-700" /> {showSpeed(currentWind)}</div>
              {typeof currentHumidity === "number" && (
                <div className="flex items-center gap-2"><Droplets className="w-4 h-4 text-sky-700" /> {Math.round(currentHumidity)}%</div>
              )}
            ...
            {wx.daily?.precipitation_probability_max?.[0] != null && (
              <p className="mt-2 text-sm text-gray-600">Chance of precip today: {wx.daily.precipitation_probability_max[0]}%</p>
            )}
          </div>
          ...
          {/* Alerts */}
          <div className="bg-white rounded-2xl shadow p-4">
            <h2 className="text-lg font-semibold text-sky-900 flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-amber-600" /> Weather Alerts</h2>
            {alerts.length === 0 ? (
              <p className="mt-2 text-sm text-gray-600">No active alerts for this area.</p>
            ) : (
          ...
          <div className="bg-white rounded-2xl shadow p-4 md:col-span-2">
            ...
            <div className="mt-3 grid md:grid-cols-5 gap-4">
              ...
              <div className="md:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                {next12.map((h, i) => (
                  <div key={i} className="rounded-xl border border-gray-200 p-2 text-gray-800">
                    <div className="text-xs text-gray-500">{fmtDate(h.t, { hour: "numeric" })}</div>
                    <div className="font-medium">{showTemp(h.temp)}</div>
                    {typeof h.pop === "number" && (
                      <div className="text-xs inline-flex items-center gap-1 text-sky-700"><CloudRain className="w-3 h-3" /> {h.pop}%</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl shadow p-4 xl:col-span-1">
              ...
              <div className="mt-2 divide-y">
                {wx.daily.time.slice(0, 5).map((d, i) => (
                  <div key={d} className="py-2 flex items-center justify-between text-gray-800">
                    <div className="w-24">{fmtDate(d, { weekday: "short", month: "short", day: "numeric" })}</div>
                    ...
                    <div className="w-40 text-right">
                      ...
                      <span className="text-gray-500"> / {showTemp(wx.daily.temperature_2m_min[i])}</span>
                    </div>
                  </div>
                ))}
              </div>
