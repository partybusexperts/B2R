...
const tools: ToolEntry[] = (registryData || []).map((d: ToolData) => {
  ...
  // default modal sizing by category when not explicitly provided
  const categoryDefaults: Record<string, string> = {
    'Capacity & Seating': 'max-w-md',
    'Compare & Choose': 'max-w-3xl',
    'Pricing & Savings': 'max-w-lg',
    'Route & Timing': 'max-w-2xl',
    'Venues & Logistics': 'max-w-2xl',
    'Policies & Risk': 'max-w-md',
    'Amenities & Experience': 'max-w-lg',
    'Planning & Coordination': 'max-w-2xl',
  };

  const modalSize = (d as any).modalSize || categoryDefaults[d.category] || 'max-w-2xl';
