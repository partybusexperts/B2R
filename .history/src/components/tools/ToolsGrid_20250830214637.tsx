...
export default function ToolsGrid({ className, limit, filter, items, randomize }: { className?: string; limit?: number; filter?: string; items?: ToolEntry[]; randomize?: boolean }) {
  ...
  const modalSizeForId = (id: string) => {
    // small, medium, large defaults — tweak as needed per-tool
  ...
  return (
    <div className={className}>
      ...
      {openIdx !== null && candidates[openIdx] && (
        <Modal open={true} onClose={closeTool} title={candidates[openIdx].title} sizeClass={modalSizeForId(candidates[openIdx].id)}>
          <div className="mb-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm text-blue-200">{candidates[openIdx].desc}</div>
              <div>
                <button type="button" onClick={() => copyEmbed(candidates[openIdx].id, openIdx)} className="text-sm bg-white/5 px-3 py-1 rounded-md hover:bg-white/10">Share</button>
              </div>
            </div>
