...
export type ToolEntry = {
  ...
  component?: React.FC<unknown>;  // optional component to render in modals/pages
  keywords?: string[];        // extra search terms
};

function makeLinkComponent(href: string, label?: string): React.FC {
  return function LinkTool() {
    // Render the linked tool inside an iframe so it opens as a modal content
    // rather than navigating the user away to a new page. This keeps the
    // UX consistent with tools that provide React components.
    return (
      <div className="space-y-2">
        <p className="text-blue-100">{label || "Open tool"}</p>
        <div className="w-full">
          <iframe src={href} title={label || href} className="w-full h-[600px] rounded-xl border border-blue-800/30" />
        </div>
      </div>
    );
  };
...
const tools: ToolEntry[] = [
  ...
  // Capacity & Seating
  { id: "capacity-finder", title: "Capacity Finder", desc: "Find vehicles by group size.", category: "Capacity & Seating", href: route("capacity-finder"), component: makeLinkComponent(route("capacity-finder"), "Open Capacity Finder") },
