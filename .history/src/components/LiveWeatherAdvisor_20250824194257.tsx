...
const LiveWeatherAdvisor: React.FC = () => {
  ...
  // ===== Boot: saved place -> approximate location -> Wheaton fallback =====
  useEffect(() => {
    const boot = async () => {
      try {
        setLoading(true);
        ...
        // Default to approximate IP location so it works on HTTP too
  ...
  const useApproxLocation = async () => {
    try {
      ...
      if (hit) {
        ...
        setGeoMessage('Could not determine location from IP. You can still search above or pin a city.');
        if (!place) {
      ...
      if (!place) {
        ...
        setGeoMessage('Network issue getting IP location. Try again or search above.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-7xl mx-auto p-6 md:p-8 bg-gradient-to-b from-sky-100 to-sky-300 min-h-screen text-gray-900">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Simplified: Only Approximate + Pin (works everywhere) */}
          <button
