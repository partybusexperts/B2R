...
export default function HeroSlideshow() {
  ...
  if (slidesTyped.length >= 2) {
    ...
    if (looksPink(last) && looksPink(prev)) {
      // Load more category images (lazy import inside component to avoid top-level change)
      try {
        const { getCategoryImages } = require('../utils/optimizedImages');
        const allParty = getCategoryImages('partyBuses');
