        ...
        try {
          ...
          const data = await resp.json();
              if (!aborted) {
                ...
                // Restore any locally stored vote for UI highlight
                try {
  ...
  return (
    <div className="bg-white rounded-xl shadow p-6 flex flex-col items-start border-l-4 border-blue-400 w-full">
      ...
      {!initialLoading && error === 'unavailable' && (
        <div className="text-xs text-red-600">Poll service offline. Votes not available.</div>
      )}
      {!initialLoading && !voted && !error && (
