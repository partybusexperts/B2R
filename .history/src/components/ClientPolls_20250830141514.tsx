    ...
    <div className="pointer-events-none fixed right-4 top-4 z-[9999] space-y-2">
      {toasts.map((t) => (
        ...
        >
          ...
          /* ===== Poll Card (same logic, themed styling) ===== */
          function PollCard({ poll, onGlobalShare }: { poll: Poll; onGlobalShare: (embed: string) => void }) {
            ...
            return (
              <div className="rounded-3xl border border-blue-800/30 bg-[#122a56] shadow-xl p-4 hover:shadow-2xl transition">
                <div className="text-[15px] font-semibold text-white leading-snug mb-3">
                  {getQuestion(poll)}
                </div>
                      ...
                      <button
                        ...
                        className={`rounded-full px-3 py-1.5 text-sm border transition ${
                          selected
                            ...
                            : "bg-[#173264] text-blue-100 border-blue-800/40 hover:border-blue-500"
                        }`}
                      >
                        {opt}
                      </button>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <button
                    ...
                    className="rounded-xl border border-blue-800/40 bg-[#173264] px-3 py-1.5 text-xs font-medium text-blue-100 hover:border-blue-500"
                  >
                    {showResults ? "Hide Results" : "View Results"}
                  </button>
                  <button
                    onClick={handleShare}
                    className="rounded-xl bg-blue-600 text-white px-3 py-1.5 text-xs font-semibold hover:bg-blue-700 border border-blue-700"
                  >
                    Share This Poll On Your Website
                  </button>
                  <span className="ml-auto text-[11px] text-blue-200/90 tabular-nums">
              ...
              <div className={`rounded-3xl border border-blue-800/30 bg-[#173264] ${compact ? "p-4" : "p-6"} shadow-lg`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-100/90">
                    Curious how people are voting? Use <span className="font-semibold text-white">Show Results</span> at the top,
                    or the <span className="font-semibold text-white">View Results</span> button on any poll. You can also{" "}
                    <span className="font-semibold text-white">share these polls on your site</span>—just click
          ...
          /* ===== Main (same functionality, all styling matched to your page) ===== */
              ...
              <div className="no-scrollbar flex gap-2 overflow-x-auto py-2">
                {data.entries.map((c) => (
                  <button
                    ...
                    className="group flex items-center gap-2 rounded-full border border-blue-800/30 bg-[#173264] px-3 py-1.5 text-sm text-blue-100 hover:border-blue-500 transition"
                    title={`${c.pretty}`}
                  >
                    <span className="font-semibold text-white">{c.pretty}</span>
                    <span className="rounded-full bg-blue-600 text-white text-[11px] px-2 py-0.5 tabular-nums border border-blue-700">
                      {c.count}
                    </span>
                  </button>
                ))}
              ...
              <main className="text-slate-100 bg-[#0f1f46] min-h-screen">
                ...
                <section className="bg-[#122a56] pt-8 pb-10 relative">
                  ...
                  <div className="max-w-7xl mx-auto px-4 md:px-6 relative">
                    <h1 className="text-4xl md:text-5xl font-extrabold text-center text-white font-serif tracking-tight">
                      Community Polls Hub
                    </h1>
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                      <div className="md:col-span-2">
                        <input
                          ...
                          placeholder="Search polls, options, or tags…"
                      ...
                      <div className="flex items-end gap-2">
                        <button
                          ...
                          className="h-[52px] w-full rounded-2xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold border border-blue-700 transition"
                        >
                          Show Results (Global)
                        </button>
                        <button
                          onClick={async () => {
                            const ok = await copyToClipboard(
                              "<!-- Paste where you want a poll to appear; replace POLL_ID -->\n<iframe src=\"https://yourdomain.com/embed/poll/POLL_ID\" width=\"100%\" height=\"420\" style=\"border:0;border-radius:12px;overflow:hidden\" loading=\"lazy\" referrerpolicy=\"no-referrer\"></iframe>"
                            );
                            if (ok) add("Copied sample embed!");
                          }}
                          className="h-[52px] whitespace-nowrap rounded-2xl bg-[#173264] px-4 text-sm font-bold text-white border border-blue-800/40 hover:border-blue-500 transition"
                        >
                          How To Embed
                        </button>
                      </div>
