...
// Seed list with human friendly labels + raw playlist IDs only; we will fetch live data when available.
...
const FALLBACK_LABEL_MAP: Record<string, string> = PLAYLIST_PRESETS.reduce((acc, p) => {
  acc[p.id] = p.fallbackLabel;
  return acc;
...
const PlaylistStarter: React.FC = () => {
  ...
  const fetchLive = useCallback(async () => {
    try {
      ...
      const mapped: PlaylistMeta[] = PLAYLIST_PRESETS.map((p) => {
        ...
        const fetched = !!raw?.fetched || raw?.status === 200 || false; // new API shape compatibility
        return {
          id: p.id,
          name: (!raw?.name || looksLikeRawId(raw.name)) ? p.fallbackLabel : raw.name,
          description: fetched ? (raw?.description || "") : "", // hide noisy placeholder
  ...
  const FallbackSmartImage: React.FC<{ src?: string; alt: string; className?: string; initials: string }> = ({
    ...
    if (!src || err) {
      return (
        <div
          className={`w-20 h-20 mb-2 rounded-lg border-2 border-blue-100 bg-gradient-to-br from-blue-200 to-blue-300 flex items-center justify-center text-2xl text-blue-700 font-bold ${className || ""}`}
          aria-label={alt}
        >
          {initials}
        </div>
    ...
    return (
      <SmartImage
        src={src}
        alt={alt}
        className={`w-20 h-20 rounded-lg mb-2 object-cover border-2 border-blue-100 ${className || ""}`}
        onError={() => setErr(true)}
      />
    );
  };
    ...
    <div className="bg-gradient-to-br from-blue-50 to-blue-200 rounded-2xl shadow-xl p-4 border-2 border-blue-400 w-full text-sm">
      ...
      {fetchError && (
        <div className="mb-4 text-xs bg-rose-100 border border-rose-300 text-rose-800 px-3 py-2 rounded-lg">
          {fetchError} — Make sure <code className="font-mono">SPOTIFY_CLIENT_ID</code> and{" "}
          <code className="font-mono">SPOTIFY_CLIENT_SECRET</code> are in <code>.env.local</code> then restart (server-only).
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {searchResults.map((r) => (
              <div key={r.id} className="bg-white rounded-xl shadow p-3 flex flex-col items-center border border-blue-200 relative">
                ...
                <div className="font-bold text-base text-blue-900 mb-1 text-center line-clamp-1" title={r.name}>
                  {r.name}
                </div>
                <div className="text-blue-800 text-center text-xs mb-1 line-clamp-2 min-h-[2.25rem]" title={r.description}>
                  {r.description}
                </div>
          ...
          return (
            <div key={pl.id} className="bg-white rounded-xl shadow p-3 flex flex-col items-center border border-blue-200 relative">
              ...
              <div className="flex flex-col w-full gap-1">
                ...
                {isPreset && !pl.live && <div className="text-[10px] text-amber-700 text-center">Fallback (no live data)</div>}
