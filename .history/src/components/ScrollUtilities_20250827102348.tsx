...
function collectSections(max = 30): TocSection[] {
  ...
  const explicit = Array.from(document.querySelectorAll<HTMLElement>('[data-section-title]'))
    .map(el => ({ id: el.id || (el.id = 'sec-' + Math.random().toString(36).slice(2)), label: el.dataset.sectionTitle || el.innerText.trim().slice(0,80) }))
    .filter(s => s.label);
  const headings = Array.from(document.querySelectorAll<HTMLElement>('main h2'))
    .map(el => ({ id: el.id || (el.id = 'h2-' + Math.random().toString(36).slice(2)), label: el.innerText.trim().replace(/#[^]*$/, '').slice(0,80) }))
    .filter(s => s.label);
  const merged: TocSection[] = [];
  const seen = new Set<string>();
  [...explicit, ...headings].forEach(s => { if (!seen.has(s.id) && merged.length < max) { seen.add(s.id); merged.push(s); }});
  return merged;
}
...
const ScrollUtilities: React.FC = () => {
  ...
  // Collect sections once after mount (delay to allow page load)
  useEffect(() => {
    const t = setTimeout(() => setSections(collectSections()), 400);
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (dockVisible) {
        if (idleRef.current) cancelIdleCallback(idleRef.current as any);
        idleRef.current = requestIdleCallback(() => setDockVisible(scrollTop > 800), { timeout: 3000 }) as any;
      }
    };
