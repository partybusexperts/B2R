        ...
        try {
          ...
          if (nomRes.ok) {
            ...
            if (Array.isArray(nom) && nom.length > 0) {
              const first = nom[0] as Record<string, any>;
              ...
              if (first.address && typeof first.address === 'object') {
                country = String(first.address.country || first.address.country_code || '');
                admin1 = String(first.address.state || first.address.county || '');
              } else {
                country = String(first.display_name || '');
                admin1 = '';
              }
              setRegion(admin1 || "");
            }
