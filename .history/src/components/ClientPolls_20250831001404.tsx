...
// Normalize raw category strings into a stable key + pretty label.
function normalizeCategory(raw: string) {
  const inStr = String(raw || "").toLowerCase().trim();
  // strip common prefixes like feature:, policy:, ops:
  const base = inStr.replace(/^[a-z]+:\s*/i, "").replace(/[_]/g, " ").trim();
  let key = base.replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "misc";
  let pretty = prettifyCategory(base);

  if (/restroom|bathroom|toilet|lavatory/.test(base)) {
    key = "amenities";
    pretty = "Amenities Polls";
  } else if (/pet/.test(base)) {
    key = "pet-policy";
    pretty = "Pet Policy Polls";
  } else if (/music|playlist|dj|song/.test(base)) {
    key = "music-playlist";
    pretty = "Music & Playlist Polls";
  } else if (/deposit|damage|cleaning|cleanup/.test(base)) {
    // deposits and cleanup-related polls grouped under vehicle deposit/cleanup
    if (/deposit/.test(base)) {
      key = "vehicle-deposit";
      pretty = "Vehicle Deposit Polls";
    } else {
      key = "cleanup";
      pretty = "Cleanup & Damage Polls";
    }
  } else if (/ada|access|wheelchair|accessible|lift|ramp|securement/.test(base)) {
    key = "ada-access";
    pretty = "ADA Access Polls";
  } else if (/pickup|drop|pickup-drop|arrival|departure|window/.test(base)) {
    key = "pickup-drop";
    pretty = "Pickup & Drop Polls";
  } else if (/smok|vape/.test(base)) {
    key = "smoking";
    pretty = "Smoking/Vaping Policy Polls";
  } else if (/extra-stop|stop(s)?/.test(base) && base.includes('extra')) {
    key = "extra-stops";
    pretty = "Extra Stops Polls";
  }

  return { key, raw: base || "Misc", pretty };
}
...
function useToasts() {
  ...
  // Stable incremental id to avoid Date.now()/Math.random() during render
...
function PollCard({ poll, onGlobalShare }: { poll: Poll; onGlobalShare: (embed: string) => void }) {
  ...
  // Compute embed code lazily when user triggers share/copy to avoid using window during SSR
  const makeEmbedCode = () => {
    const id = encodeURIComponent(String(poll.id ?? slugify(getQuestion(poll))));
    const origin = typeof window !== "undefined" ? window.location.origin : "";
    return `<iframe src="${origin}/embed/poll/${id}" width="100%" height="420" style="border:0;border-radius:12px;overflow:hidden" loading="lazy" referrerpolicy="no-referrer"></iframe>`;
  };
  ...
  const ok = await copyToClipboard(makeEmbedCode());
    if (ok) onGlobalShare("Copied!");
  };

  return (
    <div className="rounded-3xl border border-blue-800/30 bg-[#122a56] shadow-xl p-4 hover:shadow-2xl transition">
          ...
          return (
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-sm border transition ${
                selected ? "bg-blue-600 text-white border-blue-700" : "bg-[#173264] text-blue-100 border-blue-800/40 hover:border-blue-500"
              }`}
            >
              {opt}
            </button>
      ...
      <div className="flex items-center gap-2 mb-2">
        <button onClick={() => setShowResults((v) => !v)} className="rounded-xl border border-blue-800/40 bg-[#173264] px-3 py-1.5 text-xs font-medium text-blue-100 hover:border-blue-500">
        ...
        <span className="ml-auto text-[11px] text-blue-200/90 tabular-nums">{formatNumber(totalVotes)} vote{totalVotes === 1 ? "" : "s"}</span>
      </div>

      {showResults && (
        <div className="mt-2">{initialOpts.map((o) => <ResultBar key={o} label={o} pct={pctFor(o)} count={tallies[o] ?? 0} />)}</div>
      )}
    </div>
  );
}

/* ===== Inline Reminder (blue) ===== */
function ResultsReminder({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`rounded-3xl border border-blue-800/30 bg-[#173264] ${compact ? "p-4" : "p-6"} shadow-lg`}>
      <div className="flex items-start gap-3">
        ...
        <div className="text-sm text-blue-100/90">
          Curious how people are voting? Use <span className="font-semibold text-white">Show Results</span> at the top, or the <span className="font-semibold text-white">View Results</span> button on any poll. You can also <span className="font-semibold text-white">share these polls on your site</span>—just click Share.
        </div>
      </div>
    </div>
  );
...
export default function ClientPolls({ polls }: { polls: Poll[] }) {
  ...
  const data = useMemo(() => {
    // Group polls by normalized category key, but preserve raw for display/tooltips
    const groups: Record<string, { rawSamples: Set<string>; pretty: string; items: Poll[] }> = {};
    for (const p of polls || []) {
      const raw = getCategoryRaw(p);
      const norm = normalizeCategory(raw);
      const slug = norm.key;
      if (!groups[slug]) groups[slug] = { rawSamples: new Set(), pretty: norm.pretty, items: [] };
      groups[slug].rawSamples.add(norm.raw || raw || 'Misc');
      groups[slug].items.push(p);
    }

    const entries = Object.entries(groups).map(([slug, { rawSamples, pretty, items }]) => ({
      slug,
      raw: Array.from(rawSamples).slice(0,2).join(', '),
      pretty,
      items,
      count: items.length,
    }));
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 relative">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="md:col-span-2">
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search polls, options, or tags…" className="w-full rounded-full px-6 py-4 text-lg bg-[#12244e] border border-blue-800/30 text-white placeholder-blue-200 shadow focus:outline-none focus:ring-2 focus:ring-blue-500 transition" />
            </div>
