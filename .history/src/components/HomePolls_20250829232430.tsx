...
async function fetchAll(): Promise<PollsPayload> {
  ...
  const ids = polls.slice(0, 24).map((p: any) => p.id);
  let votes: Record<string, Record<string, number>> = {};
  if (ids.length) {
    try {
      const br = await fetch('/api/poll/results/bulk', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids }) });
      if (br.ok) {
...
export default function HomePolls({ groups }: { groups?: { tag: string; label?: string }[] }) {
  ...
  const groupsPicked = useMemo(() => {
    ...
    const fromSession = (tag: string) => { try { return JSON.parse(sessionStorage.getItem(`b2r_home_featured_${tag}`) || "[]") as string[]; } catch { return []; } };
    const toSession = (tag: string, ids: string[]) => { try { sessionStorage.setItem(`b2r_home_featured_${tag}`, JSON.stringify(ids)); } catch {} };
    ...
    const build = (tag: string, label?: string) => {
      ...
      if (picks.length < 8) {
        picks = picks.concat(sample(pool.filter(p => !picks.find(x => x.id === p.id)), 8 - picks.length));
        ...
        {groupsPicked.map((g, gi) => (
          <div key={gi}>
            ...
            <div className="grid grid-cols-1 gap-4">
              {g.items.slice(0,3).map(({ poll, counts }) => (
              ...
              <div className="mt-4">
                ...
                <div className="overflow-x-auto no-scrollbar">
                  <div className="flex gap-4 min-w-max">
                    {g.items.slice(3).map(({ poll, counts }) => (
