...
const PlaylistStarter: React.FC = () => {
    ...
    debounceRef.current = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!res.ok || !obj?.ok) {
          ...
          setSearchResults(success.items || []);
        }
      } catch {
  ...
  {playlists.map(pl => {
    ...
    const displayName = /^[A-Za-z0-9]{22}$/.test(pl.name || "") ? (FALLBACK_LABEL_MAP[pl.id] || pl.name) : pl.name;
          return (
        ...
        <div className="font-bold text-base text-blue-900 mb-1 text-center line-clamp-1" title={displayName}>{displayName}</div>
              ...
              <div className="flex flex-col w-full gap-1">
                {pl.url ? (
                  <div className="flex gap-2 w-full">
                    <a href={pl.url} target="_blank" rel="noopener noreferrer" className="flex-1 bg-blue-700 hover:bg-blue-800 text-white font-bold px-3 py-1 rounded-lg shadow transition text-center text-xs">Open</a>
