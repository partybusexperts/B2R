...
const LiveWeatherAdvisor = () => {
  ...
  useEffect(() => {
    const fetchCityAndWeather = async () => {
      ...
      try {
        ...
        const userCity = ipData.city || "New York";
        setCity(userCity);

        const isUS = ipData.country_code === "US";
        if (isUS) {
          const nwsResponse = await fetch(
            `https://api.weather.gov/points/${ipData.latitude},${ipData.longitude}`
          );
          ...
          const alertsResponse = await fetch(
            `https://api.weather.gov/alerts/active?point=${ipData.latitude},${ipData.longitude}`
          );
          const alertsData = await alertsResponse.json();

          setWeather({
            current: {
              temperature: (forecastData.properties.periods[0].temperature - 32) * (5 / 9),
              weathercode: mapNWStoWMO(forecastData.properties.periods[0].shortForecast),
          ...
          const meteoResponse = await fetch(
            `https://api.open-meteo.com/v1/forecast?latitude=${ipData.latitude}&longitude=${ipData.longitude}&current=temperature_2m,weathercode&daily=temperature_2m_max,temperature_2m_min,weathercode,sunrise,sunset&hourly=uv_index,pm25&timezone=auto`
          );
  ...
  const getWeatherTips = (weathercode: number, temp: number): string[] => {
    const tips: string[] = [];
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-gradient-to-b from-sky-50 to-sky-100 min-h-screen overflow-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow-md hover:scale-105 transition-transform">
            ...
            <ul className="list-disc list-inside text-gray-800 text-sm">
              {getWeatherTips(weather.current.weathercode, weather.current.temperature).map(
                (tip, index) => (
                  <li key={index}>{tip}</li>
                )
              )}
