...
async function main() {
  ...
  if (!pollsRes.data || pollsRes.data.length === 0) {
    const sampleTags = await supabase
      .from("v_polls_with_tags")
      .select("tag_slug")
    ...
    if (sampleTags.error) {
      console.error("Sample tag error:", sampleTags.error);
    } else {
      const tagList = Array.from(
        new Set(sampleTags.data.map((row) => row.tag_slug))
      );
      console.log("Sample tag slugs:", tagList);
    }
    return;
  }
