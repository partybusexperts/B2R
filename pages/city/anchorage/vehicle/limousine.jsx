// pages/city/anchorage/vehicle/limousine.jsx
...
export async function getStaticProps() {
  ...
  const seo = {
    title: `${city?.name || 'Anchorage'} Limousine Service — Bus2Ride`,
    description: `Private limousines in ${city?.name || 'Anchorage'}. Compare vehicles, photos, and get a quote.`,
  };
...
export default function AnchorageLimousine({ city, vehiclesList, images, polls, options, blocks, amenities, seo }) {
  return (
    <>
      <Head>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem' }}>
        <header>
          <h1>Limousines in {city?.name || 'Anchorage'}</h1>
          <p>Private limo rentals for weddings, proms, and executive travel — get a quote.</p>
        </header>
        ...
        <section style={{ marginTop: 24 }}>
          ...
          {vehiclesList.length === 0 ? <p>No limousines found for Anchorage yet.</p> : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 12 }}>
              {vehiclesList.map(v => (
                <article key={v.id} style={{ border: '1px solid #eee', padding: 12, borderRadius: 8 }}>
                  ...
                  <a href={`/get-quote?city=${city?.slug}&vehicle=limousine&vehicle_id=${v.id}`} className="btn">Get quote</a>
                </article>
              ))}
            </div>
          )}
