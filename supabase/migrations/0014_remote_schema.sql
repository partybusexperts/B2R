...
alter table "public"."heroes1" enable row level security;
  ...
  create table "public"."homepage_hero" (
    ...
    "title" text not null default 'Bus2Ride ???????? Group transport made easy'::text,
...
begin
  ...
  perform upsert_poll(
    p_title           => format('%s bookings: what????????s your preferred method?', _city_name),
...
begin
  ...
  for i in 1..n loop
    title := format('%s ??????? Question %s', cat_name, to_char(i,'FM00'));
...
begin
  ...
  -- SUBJECTS + FRAMES (short lists x short lists ???????? 50+ unique combos)
  if fam='vehicle' then
    subjects := array[
...
AS $function$
  select case
    ...
    when cat_slug = 'deposit-polls'              then array['No deposit','< 10%','10????????25%','25%+']
...
begin
  ...
  loop
    ...
    -- ??????? Use RETURN QUERY for RETURNS TABLE
    return query
