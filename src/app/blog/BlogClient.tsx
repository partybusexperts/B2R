...
export default function BlogClient() {
  ...
  const renderVehicleCards = (images: string[], href: string, label: string) => (
    <div className="grid md:grid-cols-3 gap-6 w-full">
      {images.map((img, i) => (
        <div key={img} className="bg-white rounded-2xl shadow-xl p-4 flex flex-col items-center">
          <a href={href} className="block w-full group" aria-label={`View ${label} fleet`}>
            <SmartImage
              src={img}
              alt={label}
              className="w-full h-60 object-cover rounded-2xl mb-4 group-hover:opacity-90 transition"
              sizes="(max-width:640px) 100vw, (max-width:1024px) 50vw, 33vw"
            />
            <h4 className="text-base font-bold mb-2 text-blue-800">{label} {i + 1}</h4>
          </a>
          <div className="flex flex-col gap-2 w-full mt-auto">
            <a href={`tel:${PHONE_TEL}`} className="w-full bg-blue-700 text-white font-bold py-1 rounded-lg text-center text-base font-serif">
              {PHONE_DISPLAY}
            </a>
            <a
              href={`mailto:${EMAIL}`}
              className="w-full bg-blue-500 text-white font-bold py-1 rounded-lg text-center text-base font-serif hover:bg-blue-600"
            >
              Email Us
            </a>
            <a href="/quote" className="w-full bg-green-500 text-white font-bold py-1 rounded-lg text-center text-base font-serif hover:bg-green-600">
              Instant Live Quote
            </a>
          </div>
        </div>
    ...
    <>
      ...
      <Section className="max-w-6xl mx-auto bg-gradient-to-br from-blue-900/80 to-black mb-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 max-w-5xl mx-auto">
          {[
            {
              quote: "Absolutely excellent! Great customer service, spotless vehicles, and super professional drivers—made our night effortless!",
              name: "Paul P.",
            },
            {
              quote: "Best price and experience we’ve had—party bus was immaculate and the team was beyond accommodating.",
              name: "Jessie A.",
            },
            {
              quote: "We booked for a bachelor + wedding weekend—flawless both times. Extended an hour on the fly. Total pros.",
              name: "Dee C.",
            },
            {
              quote: "Driver was friendly, on time, and the interior was pristine. Won’t use anyone else now!",
              name: "Halee H.",
            },
          ].map((r, i) => (
        ...
        <div className="flex justify-center mt-4">
          <a href="/reviews" className="inline-block bg-blue-700 hover:bg-blue-800 text-white font-bold px-10 py-4 rounded-2xl shadow-lg text-lg border border-blue-800/50 transition">
            More Reviews
          </a>
        </div>
      </Section>
