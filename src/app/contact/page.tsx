...
async function fetchContactHero() {
  ...
  try {
    ...
    // Use the same server RPC as the client to keep data source consistent
    const { data, error } = await supabase.rpc('fetch_header', { p_page_slug: 'contact' });
    if (error || !data) return null;

    // rpc returns a single row (matching the RETURNS TABLE signature)
    const row = Array.isArray(data) ? data[0] : data;
    const candidate = row?.body ?? row?.data ?? row ?? null;
