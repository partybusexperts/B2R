...
const legacyFallbacks = ['/images/party-buses/18 Passenger White Party Bus Exterior.png', '/images/party-buses/30 Passenger Party Bus Exterior.png', '/images/party-buses/36 Passenger Party Bus Inside.png'];
    ...
    <>
      <HeroHeader
        ...
        fallback={{
          ...
          subtitle: "Explore popular event types—weddings, proms, concerts, game days & more—then jump straight to the one you need.",
              ...
              const card = (
                ...
                >
                  ...
                  <div className="flex flex-row flex-wrap gap-3 justify-center items-center w-full mt-auto">
                    ...
                    {/* Related Polls (links to polls filtered by event tag) */}
                    <a
                      href={`/polls?tag=${encodeURIComponent(event.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, ""))}`}
                      className="rounded-full font-bold px-5 py-2 text-sm tracking-tight shadow-md transition border flex items-center justify-center min-w-[140px] bg-blue-700 text-white border-blue-700 hover:bg-blue-800"
                    >
                      Related Polls
                    </a>
                  </div>
