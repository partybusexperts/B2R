...
// Uniform CTA styles for all buttons (same height & min-width)
const CTA = {
  base:
  ...
  return (
    <>
      ...
      <div className="w-full bg-[#122a56] py-12 px-3 md:px-6">
        <div className="w-full max-w-7xl mx-auto">
          <div className="w-full flex flex-col md:flex-row items-center justify-center gap-3 mb-10">
            ...
            {/* Type-ahead search filters the grid & the select */}
            <input
              id="event-search"
            ...
            {/* Select stays in sync with the filtered list */}
            <select
              className="rounded-lg px-4 py-2 bg-[#0f1f46] text-blue-50 border border-blue-800/40 focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[240px]"
              value={selectedEvent}
            ...
            >
              ...
              {filtered.map((event) => (
                <option
                  key={event.name}
                  value={event.href || `/events/${slugify(event.name)}`}
                >
                  {event.name}
                </option>
              ))}
            ...
            {filtered.map((event, i) => {
              ...
              const card = (
                <a
                  ...
                  className="relative bg-[#0f1f46] rounded-3xl border border-blue-800/40 p-6 no-underline min-h-[480px] flex flex-col items-center shadow-[0_10px_28px_-4px_rgba(0,0,0,.45)] transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-[0_18px_36px_-6px_rgba(0,0,0,.55)] group
                  before:absolute before:inset-0 before:rounded-3xl before:p-[1.5px] before:bg-gradient-to-r before:from-blue-500/20 before:via-cyan-400/20 before:to-indigo-400/20 before:opacity-0 group-hover:before:opacity-100 before:transition-opacity before:duration-200"
                >
                  <SmartImage
      ...
      <script
        ...
        // biome-ignore lint/security/noDangerouslySetInnerHtml: intentional JSON-LD
