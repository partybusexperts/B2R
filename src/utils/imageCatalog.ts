...
// Deterministic pseudo-random shuffle with optional seed to avoid hydration mismatch
export function getRandomImages(category: VehicleCategory, count: number, seed?: string): VehicleImage[] {
  const arr = [...imageCatalog[category]];
  if (arr.length <= 1 || count <= 0) return arr.slice(0, Math.max(0, count));
  let h: number;
  if (seed) {
    // FNV-1a hash
    h = 2166136261 >>> 0;
  ...
  return keyed.slice(0, Math.max(0, count)).map(x => x.img);
}
