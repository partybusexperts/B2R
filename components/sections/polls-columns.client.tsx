        ...
        <div key={colIdx} className="relative">
          <div
            ...
            className="h-[675px] overflow-y-scroll snap-y snap-mandatory
              rounded-3xl border border-white/10 bg-white/5 polls-column-scroll"
          >
            {col.map((poll, idx) => (
              <div
                key={`${poll.id}-${idx}`}
          ...
          {(() => {
            ...
            const thumbHeight = Math.max(
              28,
              Math.round((meta.clientHeight / meta.scrollHeight) * trackHeight),
            );
            const thumbTop =
              trackPadding +
              Math.round(
            ...
            const onThumbPointerDown = (
              ...
              const pointerOffsetInThumb = e.clientY - trackRect.top - thumbTop;
